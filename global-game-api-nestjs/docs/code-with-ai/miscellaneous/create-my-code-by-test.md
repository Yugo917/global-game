# Create-my-code-by-test.md

# Request
```
Can you create the logInterpolator regarding the test suite below
---
...
# Response
```Ts
export class LogInterpolator {
    
    /**
     * Replaces placeholders in the template string with values from the metadata.
     * Placeholders are in the format {key}.
     */
    interpolateMessage(template: string, metadata: Record<string, any>): string {
        return template.replace(/{(\w+)}/g, (match, key) => {
            if (key in metadata) {
                const value = metadata[key];
                return typeof value === 'object' ? JSON.stringify(value) : value;
            }
            return match; // Leave the placeholder intact if key is not found
        });
    }

    /**
     * Converts all values in the metadata to strings, including objects and arrays by serializing them into JSON.
     */
    toMetaDataValuesAsString(metadata: Record<string, any>): Record<string, string> {
